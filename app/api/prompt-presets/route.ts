...
export async function POST(request: Request) {
  try {
    const { name, system_prompt, test_prompt, blog_fetch_prompt, model, blog_fetch_model, style_agent_model, is_default } = await request.json();
    if (!name || !system_prompt || !test_prompt) {
      return Response.json({ error: "name, system_prompt, and test_prompt are required" }, { status: 400 });
    }
    const preset = await insertPromptPreset({ name, system_prompt, test_prompt, blog_fetch_prompt, model, blog_fetch_model, style_agent_model, is_default });
...
export async function PUT(request: Request) {
  try {
    const { id, name, system_prompt, test_prompt, blog_fetch_prompt, model, blog_fetch_model, style_agent_model, setDefault } = await request.json();
    ...
    const preset = await updatePromptPreset(id, { name, system_prompt, test_prompt, blog_fetch_prompt, model: model || "openai/gpt-4o", blog_fetch_model, style_agent_model });
    return Response.json({ preset });
  } catch (error) {
